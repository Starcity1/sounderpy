...
def __metpy_sounding(clean_data):
    ...
    # David Rodriguez Sanchez - Adding county lines behind sounding plot.
    cax = fig.add_axes(projection = ccrs.PlateCarree(), rect=(0, 0, 0.6, 1))
    cax.set_box_aspect(1)
    cax.set_zorder(10)
    cax.set_extent([latlon1[1] - .5, latlon1[1] + .5, latlon1[0] -.5, latlon1[0] + .5], ccrs.PlateCarree())
    cax.add_feature(USCOUNTIES.with_scale('5m'))
    cax.patch.set_alpha(0)

    skew = SkewT(fig, rotation=43, rect=(0, 0, 0.6, 1)) 
    # Change to adjust data limits and give it a semblance of what we want
    skew.ax.set_zorder(0)
    ...
    skew.plot_barbs(pressure=p[idx],u=blank,v=blank,xloc=0.955 + .045, fill_empty=True,
                    sizes=dict(emptybarb=0.075, width=0.18, height=0.4))
    skew.plot_barbs(pressure=p[idx], u=u[idx], v=v[idx],xloc=0.955 + .045, fill_empty=True,
